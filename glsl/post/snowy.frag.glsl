...
{
    float speed=1;
    vec2 iResolution = textureSize(u_RenderedTexture, 0);
    ...
    vec3 sColor = vec3(Snowout*0.9, Snowout, Snowout*1.1);

    // background layer color
    vec3 bColor = vec3(texture(u_RenderedTexture, fs_UV));
    // make the background layer color grayer
    // Convert the color of this fragment into grey scale.
    float grey = 0.21 * bColor.r + 0.72 * bColor.g + 0.07 * bColor.b;
    // construct the gray color
    vec3 grayColor = vec3(grey, grey, grey);
    // get a gray scale rate to clamp;
    float gRate = 0.4;
    // clamp the color
    bColor = mix(bColor, grayColor, gRate);

    // blend background layer color and the snow layer color
    vec3 mixColor = sColor + bColor;
